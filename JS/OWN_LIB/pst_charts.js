...
//</SF>
	...
	//<nn>
	// ÍVégül,de nem utolsó sorban, a jelző elemek (oszlopok, vonalak, körcikkek, stb) legenrálása.
	//</nn>
	PSTCG.CRT_DATAOBJS(svg,"","","","");
...
// Szerző:  Balise Pascal
// Az adatokat reprezentáló SVG objektumok legenerálása.<br>
// PARAMÉTEREK:
//×-
...
//</SF>
	...
	//<nn>
	// Itt is beerőltetjük az összes generálandó elemet egy group-ba 
	//</nn>
	var drawObjs = $(document.createElementNS(PSTCG_CNSTS.SVGNS, 'g'));
	
	
	
	for(var ix1=0; ix1<data.length; ix1++){
		var actDrwObj = $(document.createElementNS(PSTCG_CNSTS.SVGNS, 'rect'));
		console.log("x: "+ (5+(barWidth*ix1)));
		actDrwObj.attr({
			"id":"dataObj_"+type+"_"+ix1,
			"x":PSTCG_CNSTS.DEFS.BASE_S_SVG_MRG_L+(barWidth*ix1)+(5*ix1)+(barWidth/2),
			"y":PSTCG_CNSTS.DEFS.BASE_S_SVG_W - PSTCG_CNSTS.DEFS.BASE_S_SVG_MRG_R-data[ix1],
			"width":barWidth,
			"height":data[ix1],
			"fill":"#FFFF00",
			"stroke":"#555511",
